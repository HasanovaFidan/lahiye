...
function Basket() {
  ...
  const createOrder = (items, totalOrderAmount) => {
    axios.post("http://localhost:8080/order", { items: items, totalOrderAmount: totalOrderAmount }).then(res => {
      console.log(res);
      // Sepeti temizleme işlemi (isteğe bağlı)
      // clearBasket();
      // Ardından "/order" sayfasına yönlendirme
      ...
      <div className="sebet">
        ...
        <div className="right-order">
          ...
          <div className="order">
            ...
            <button onClick={() => { createOrder(basket, totalOrderAmount) }} className='orderbutton'>Sifarişi Rəsmiləşdir</button>
          </div>
        </div>
      </div>
    </div>
